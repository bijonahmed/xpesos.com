                        ...
                        <div class="row">
                            <div class="col-md">
                                <a href="#" data-toggle="modal" data-target="#modal-animation-3"> <i
                                        class="fa fa-plus"></i> Create a in Submenu </a>
                            </div>
            ...
            <div class="modal-content animated">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="Xlarge-input" class="col-form-label">Sub Menu Name (English) </label>
                        <select id="sub_menu_id" name="sub_menu_id" style="width: 100%;">
                            <option value="">Select Sub Menu</option>
                            <?php
                                    foreach ($submenu as $val) {
                                        ?>
                            <option value="<?php echo $val->sub_menu_id; ?>"><?php echo $val->name; ?>
                            </option>
                            <?php
                                    }
                                    ?>
                    ...
                    <div class="form-group">
                        ...
                        <input type="hidden" class="form-control" id="sub_in_sub_id" name="sub_in_sub_id">
...
$(".btn-submit").click(function(e) {
    ...
    var sub_menu_id = $("select[name=sub_menu_id]").val();
    var sub_in_sub_id = $("input[name=sub_in_sub_id]").val();
    if (name == '' && slug == '') {
        alert("Please insert your sub menu in sub name or slug");
    }
    $.ajax({
        type: 'POST',
        url: "save-sub-menu-in-sub",
        dataType: "json",
        data: {
            _token: _token,
            name: name,
...
function getbyId(sub_in_sub_id) {
    ...
    $.ajax({
        type: 'GET',
        dataType: "json",
        url: "menu/searchbysubmenuin/" + sub_in_sub_id,
        data: {
            "sub_in_sub_id": sub_in_sub_id,
            "_token": _token,
        },
...
$(document).ready(function() {
    //  alert("test");
    featch_sub_menu_in_list();

    function featch_sub_menu_in_list(query = '') {
        console.log("test");
        $.ajax({
            url: "{{ route('listBySubinMenu.search') }}",
            method: 'GET',
            data: {
                query: query
            },
