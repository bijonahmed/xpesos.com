	...
	<div class="row">
		<div class="col-lg-12">
			<div class="card" style="padding: 10px;">
				...
				<form method="post" action="{{ url('admin/filterProducts')}}">
					<!-- Tab panes -->
						...
						<div id="tabe-13" class="container-fluid tab-pane active">
							...
							<div class="row">
								...
								<div class="col-md-3">
									...
									<button class="btn btn-danger btn-block" onclick="downloadApiData();" id="import">
										<i aria-hidden="true" class="fa fa-refresh"></i> &nbsp;Syntronic</button>

								</div>
								<input type="hidden" name="maxlimit" value="500" id="maxlimit" />
								...
								<div class="col-md-3">
									...
									<div class="form-group" style="text-align: right;">
										...
										<input type="hidden" name="maxLimit" id="maxLimit" value="500" placeholder="Max Limit" />
									</div>
...
<script>
	...
	function featch_list(query = '') {
		$("#loader").show();
		var selectBatch = $("#filterBatch").val();
	...
	function downloadApiData() {
		...
		if (selectBatch == "") {
			...
			$.ajax({
				url: "{{ route('apilistByProduct.search') }}",
				data: {
					"selectBatch": selectBatch,
					"maxlimit": maxlimit,
					"_token": _token,
	...
	function downloadCategory() {
		...
		$.ajax({
			url: "{{ route('downloadCategory.search') }}",
			data: {
				"selectBatch": selectBatch,
				"maxlimit": maxlimit,
				"_token": _token,
			...
			dataType: 'json',
			beforeSend: function() {
				$('#showcattext').html('<b><h4>Matching Step 1 ..</b></h4>'); //showcattext
				$('#import').attr('disabled', 'disabled');
			},
			success: function(data) {
				$("#catloader").hide();
	...
	function downloadSubCategory() {
		...
		$("#catloader").show();
		$.ajax({
			url: "{{ route('downloadsubCategory.search') }}",
			data: {
				"selectBatch": selectBatch,
				"maxlimit": maxlimit,
				"_token": _token,
			...
			dataType: 'json',
			beforeSend: function() {
				$('#showcattext').html('<b><h4>Matching Step 2 ..</b></h4>'); //showcattext
				$('#import').attr('disabled', 'disabled');
			},
			success: function(data) {
				$("#catloader").hide();
	...
	function downloadInSubCategory() {
		...
		$("#catloader").show();
		$.ajax({
			url: "{{ route('getInsertInSubCategorys.search') }}",
			data: {
				"selectBatch": selectBatch,
				"maxlimit": maxlimit,
				"_token": _token,
			...
			dataType: 'json',
			beforeSend: function() {
				$('#showcattext').html('<b><h4>Matching Step 3 ..</b></h4>'); //showcattext
				$('#import').attr('disabled', 'disabled');
			},
			success: function(data) {
				$("#catloader").hide();
