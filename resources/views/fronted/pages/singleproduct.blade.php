                    ...
                    <div class="ps-product__header ps-product__box">
                        <div class="ps-product__thumbnail" data-vertical="true">
                                ...
                                <div class="ps-wrapper">
                                    ...
                                    <?php
                                        if (!empty($product->photo1)) {
                                            ?>
                                        <div class="item"><a href="{{ url('admin/'.$product->photo1) }}"><img src="{{ url('admin/'.$product->photo1) }}" alt="{{$product->product_name}}"></a></div>
                                        <?php } ?>

                                        <?php
                                        if (!empty($product->photo2)) {
                                            ?>
                                        <div class="item"><a href="{{ url('admin/'.$product->photo1) }}"><img src="{{ url('admin/'.$product->photo2) }}" alt="{{$product->product_name}}"></a></div>
                                        <?php } ?>

                                        <?php
                                        if (!empty($product->photo3)) {
                                            ?>
                                        <div class="item"><a href="{{ url('admin/'.$product->photo3) }}"><img src="{{ url('admin/'.$product->photo1) }}" alt="{{$product->product_name}}"></a></div>
                                        <?php } ?>

                                        <?php
                                        if (!empty($product->photo4)) {
                                            ?>
                                        <div class="item"><a href="{{ url('admin/'.$product->photo4) }}"><img src="{{ url('admin/'.$product->photo1) }}" alt="{{$product->product_name}}"></a></div>
                                        <?php } ?>

                                        <?php
                                        if (!empty($product->photo5)) {
                                            ?>
                                        <div class="item"><a href="{{ url('admin/'.$product->photo5) }}"><img src="{{ url('admin/'.$product->photo1) }}" alt="{{$product->product_name}}"></a></div>
                                        <?php } ?>
                            ...
                            <?php if (!empty($product->photo1)) {
                                            ?>
                                <div class="item"><img src="{{ url('admin/'.$product->photo1) }}" alt="{{$product->product_name}}"></div>
                                <?php } ?>

                                <?php if (!empty($product->photo2)) {
                                            ?>
                                <div class="item"><img src="{{ url('admin/'.$product->photo2) }}" alt="{{$product->product_name}}"></div>
                                <?php } ?>

                                <?php if (!empty($product->photo3)) {
                                            ?>
                                <div class="item"><img src="{{ url('admin/'.$product->photo3) }}" alt="{{$product->product_name}}"></div>
                                <?php } ?>

                                <?php if (!empty($product->photo4)) {
                                            ?>
                                <div class="item"><img src="{{ url('admin/'.$product->photo4) }}" alt="{{$product->product_name}}"></div>
                                <?php } ?>

                                <?php if (!empty($product->photo5)) {
                                            ?>
                                <div class="item"><img src="{{ url('admin/'.$product->photo5) }}" alt="{{$product->product_name}}"></div>
                                <?php } ?>
                        ...
                        <div class="ps-product__info">
                            ...
                            <div id="fb-root"></div>
                                        ...
                                        <div class="fb-like custom-button btn btn-block"
                                            data-href="https://www.facebook.com/KaruGhorMegaShop/?epa=SEARCH_BOX"
    ...
    <?php } ?>
    <?php }elseif ($product->pro_option == 3) { ?>
    ...
    <button class="ps-btn btn btn-primary btn-block" onclick="bottomordermodal();"
        style="background-color: #4169E1; color: white;"><b>Single Order</b></button>
    <?php } else if ($product->pro_option == 3) {  ?>
    <button class="ps-btn btn btn-primary btn-block" style="background-color: #4169E1; color: white;"
        onclick="cartShoe();"><b>Single Order</b></button>
    <?php }else{ ?>
     <button class="ps-btn btn btn-primary btn-block" style="background-color: #4169E1; color: white;"
        onclick="ordermodal();"><b>Single Order</b></button>
    <?php } ?>
    </div>
                            </div>
                            ...
                            <div class="ps-product__shopping">
                                ...
                                <?php } else if ($product->pro_option == 2) { ?>
                                    <a class="ps-btn ps-btn--black" onclick="bottomordermodaladdtoCart();" >Add to cart</a>
                                     <?php } else if ($product->pro_option == 3) { ?>
                                    <a class="ps-btn ps-btn--black" onclick="cartShoe();" >Add to cart</a>
                                    <?php } else { ?>
                ...
                <div class="ps-section--default" style="background-color: white;">
                    ...
                    <div class="ps-section__content">
                        ...
                        <?php
                            ...
                            <div class="ps-product" style="box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);">
                                <div class="ps-product__thumbnail">
                                <a href="{{url('/product/'.$item->slug)}}">
                                <img src="{{ url('admin/'.$item->photo1) }}" alt="{{$item->product_name}}" style="height: 200px; width: 100%;" ></a>
                                
                                </div>
                                <div class="ps-product__container"><a class="ps-product__vendor" href="#">Bijon's Store</a>
                                    <div class="ps-product__content"><a class="ps-product__title" href="{{url('/product/'.$item->slug)}}">
                                    {{$item->product_name}}</a>
                                    ...
                                    <div class="ps-product__content hover"><a class="ps-product__title" href="{{url('/product/'.$item->slug)}}">{{$item->product_name}}</a>
        ...
        <div class="modal" tabindex="-1" role="dialog" id="message">
                ...
                <!-- Modal -->
                <div class="modal fade-scale animate" id="myModal" tabindex="-1" role="dialog"
                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    ...
                                    <input type="text" name="mobile_number" id="mobile_number" placeholder="01xxxxxxxxx"
                                        maxlength="11" class="form-control" required onkeyup="if (/\D/g.test(this.value))
                                                    this.value = this.value.replace(/\D/g, '');
                                                enableordernow();" />
                                    <span>Enter your name & shipping address. </span>
                                    <textarea class="form-control" name="shipping_details" id="shipping_details"
                                        requied></textarea>
                                </div>
                                <div class="modal-footer" >
                                    <button type="submit" id="order" class="btn btn-primary btn-submit btn-block" style="font-size: 18px;"><i
                                            class="fa fa-check-square-o"></i> Place Order
                                    </button>
                                </div>
                            </div>
                        </div>
...
<script src="{{url(url('admin/urls/js/jquery.min.js'))}}"></script>
            ...
            (function(d, s, id) {
             ...
             function cartShoe(){
                var shoe_size = $("#shoe_size").val(); //$("input[name='bottomoptradio']:checked").val();
                if (shoe_size === "") {
                    $('#error').modal('show');
                } else {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            location.replace("{{ url('/show-cart') }}");
                        }
                    });
                }
    }
